...
namespace test_impl {
    ...
    public:
        ...
        ~R()
        {
            if(!dismissed_ && eval_(42)) { // NOLINT(readability-magic-numbers)
                this->oops(42);            // NOLINT(readability-magic-numbers)
            }
        }
        template<typename U>
...
{
    return ::test_impl::test_failures() != 0 ? EXIT_FAILURE : EXIT_SUCCESS;
}
