...
struct sentinel {
	...
	friend iterator::difference_type operator-(iterator, sentinel);
};

namespace cjdb {
	template<>
	inline constexpr auto disable_sized_sentinel<sentinel, iterator> = true;
} // namespace cjdb

static_assert(cjdb::input_iterator<iterator>);
static_assert(cjdb::sentinel_for<sentinel, iterator>);
static_assert(cjdb::disable_sized_sentinel<sentinel, iterator>);
