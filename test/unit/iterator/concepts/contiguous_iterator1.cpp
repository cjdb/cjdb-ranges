...
namespace std {
	template<>
	struct pointer_traits<::iterator> {
		using pointer = dummy*;
		using element_type = cjdb::iter_value_t<::iterator>;
		using difference_type = cjdb::iter_difference_t<::iterator>;
...
static_assert(cjdb::contiguous_iterator<iterator>);
